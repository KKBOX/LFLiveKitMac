...
bool LFNALUnit::Parse(const BYTE *pBuffer, int cSpace, int LengthSize, bool bEnd) {
...
void LFNALUnit::ResetBitstream() {
	m_idx = 0;
	m_nBits = 0;
	m_cZeros = 0;
}

void LFNALUnit::Skip(int nBits) {
	if (nBits < m_nBits) {
		m_nBits -= nBits;
	}
	else {
...
unsigned long LFNALUnit::GetBit() {
	if (m_nBits == 0) {
		m_byte = GetBYTE();
		m_nBits = 8;
	}
...
unsigned long LFNALUnit::GetWord(int nBits) {
	unsigned long u = 0;
	while (nBits > 0) {
		u <<= 1;
		u |= GetBit();
...
LFSeqParamSet::LFSeqParamSet() : m_cx(0), m_cy(0), m_FrameBits(0) {
	// SetRect(&m_rcFrame, 0, 0, 0, 0);
}

void ScalingList(int size, LFNALUnit *pnalu) {
	long lastScale = 8;
	long nextScale = 8;
...
bool LFSeqParamSet::Parse(LFNALUnit *pnalu) {
...
// --- slice header --------------------
bool LFSliceHeader::Parse(LFNALUnit *pnalu) {
	switch (pnalu->Type()) {
