...
{
	[aCoder encodeObject:[NSValue valueWithSize:self.videoSize] forKey:@"videoSize"];
...
{
	self = [super init];
	_videoSize = [[aDecoder decodeObjectForKey:@"videoSize"] sizeValue];
...
{
	NSUInteger hash = 0;
	NSArray *values = @[[NSValue valueWithSize:self.videoSize], @(self.videoFrameRate), @(self.videoMaxFrameRate), @(self.videoMinFrameRate), @(self.videoMaxKeyframeInterval), @(self.videoBitRate), @(self.videoMaxBitRate), @(self.videoMinBitRate), self.avSessionPreset, @(self.sessionPreset),
//                        @(self.outputImageOrientation),
			@(self.autorotate), @(self.videoSizeRespectingAspectRatio)];
